  ...
  const fetchStudentMap = async () => {
    try {
        const response = await axios.get("https://skyblue-clam-769210.hostingersite.com/getmapuser.php", {
            params: { mapType: "student" }, // Pass the map type as 'student'
            headers: {
                "Content-Type": "application/json",
            },
            withCredentials: true, // Include credentials for session validation
        ...
        console.error("Error fetching the student map:", err);
...
// Fetch the map on component mount
useEffect(() => {
    fetchStudentMap();
}, []);
        ...
        >
          ...
          <nav className={`bg-white z-30 rounded-r-2xl drop-shadow-2xl absolute inset-y-0 left-0 transform xl:w-1/5 lg:relative lg:translate-x-0 lg:top-0 lg:w-1/4 lg:h-screen lg:flex lg:flex-col lg:items-center lg:justify-around lg:overflow-y-auto max-sm:flex max-sm:flex-col max-sm:items-center max-sm:justify-around max-md:flex max-md:flex-col max-md:justify-around max-md:items-center md:flex md:flex-col md:justify-around md:items-center ${isNavOpen ? 'block w-full' : 'max-sm:hidden md:hidden max-md:hidden'}`}>
            ...
            <Link to="/studentparkingslot" className="group no-underline h-14 flex items-center pl-8 bg-blue-700 hover:bg-blue-700 mb-2 duration-200 lg:pl-3">
              <li className="group-hover:text-white border-l-2 border-white pl-5 text-lg text-white tracking-widest flex items-center w-full lg:text-base xl:text-lg ml-4">
